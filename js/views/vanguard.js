...
export async function init(container, user) {
  ...
  // ── State ─────────────────────────────────────────────────────────────────────
  let logs = {}, latestPredictions = [], currentModalExportData = null, currentCycleStart = null
  ...
  // ── Utils ─────────────────────────────────────────────────────────────────────
  function saveToLocal() { localStorage.setItem('vanguard-logs', JSON.stringify(logs)); saveCloudKey('vanguard-logs', logs); _nexusSync.broadcast('VANGUARD') }
  function loadFromLocal() { const d = localStorage.getItem('vanguard-logs'); if (d) try { logs = JSON.parse(d) } catch { logs = {} } }

  let cycleGoals = {}
  function saveCycleGoals() { localStorage.setItem('vanguard-cycle-goals', JSON.stringify(cycleGoals)); saveCloudKey('vanguard-cycle-goals', cycleGoals) }
  ...
  function formatHumanReadable(d) { return d.toLocaleDateString('en-US', { month:'short', day:'2-digit', year:'numeric' }) }
  ...
  function getCycleDays(cs) {
    ...
    return Math.round((Date.UTC(ce.getFullYear(),ce.getMonth(),ce.getDate()) - Date.UTC(cs.getFullYear(),cs.getMonth(),cs.getDate())) / 86400000) + 1
  }
  function getCycleNumber(cs) { return cs.getDate() <= 10 ? 1 : cs.getDate() <= 20 ? 2 : 3 }
  ...
  function isoWeekNum(date) {
    ...
    const weekStart = new Date(d); weekStart.setUTCDate(d.getUTCDate() - dayOfWeek)
    const jan1 = new Date(Date.UTC(weekStart.getUTCFullYear(), 0, 1))
    const week1Start = new Date(jan1); week1Start.setUTCDate(jan1.getUTCDate() - jan1.getUTCDay())
    return Math.floor((weekStart - week1Start) / (7 * 24 * 3600 * 1000)) + 1
  }

  // ── AI Prediction ─────────────────────────────────────────────────────────────
  function calculateAI_Predictions() {
    const today = getPSTDate(), tomorrow = new Date(today); tomorrow.setDate(tomorrow.getDate() + 1)
    const tomDow = tomorrow.getDay(), todayStr = formatPSTDate(today)
    const window14 = []
    for (let i = 1; i <= 14; i++) { const d = new Date(today); d.setDate(d.getDate() - i); window14.push(formatPSTDate(d)) }
    const logged14 = window14.filter(ds => logs[ds])

    const stats = MISSIONS.map(m => {
      let baseSum = 0
      logged14.forEach(ds => { const st = logs[ds]?.missions?.[m.id]; if (st === 'completed') baseSum += 1; else if (st === 'partial') baseSum += 0.5 })
      const baseRate = logged14.length > 0 ? baseSum / logged14.length : null
      const dowDays  = logged14.filter(ds => { const [y,mo,d] = ds.split('-').map(Number); return new Date(y,mo-1,d).getDay() === tomDow })
      let dowSum = 0; dowDays.forEach(ds => { const st = logs[ds]?.missions?.[m.id]; if (st === 'completed') dowSum += 1; else if (st === 'partial') dowSum += 0.5 })
      const dowRate = dowDays.length >= 2 ? dowSum / dowDays.length : null
      let streakSum = 0
      for (let i = 1; i <= 3; i++) { const d = new Date(today); d.setDate(d.getDate() - i); const st = logs[formatPSTDate(d)]?.missions?.[m.id]; if (st === 'completed') streakSum += 1; else if (st === 'partial') streakSum += 0.5 }
      const todaySt = logs[todayStr]?.missions?.[m.id]
      const todayBonus = todaySt === 'completed' ? 0.12 : todaySt === 'partial' ? 0.06 : 0
      let prob
      if (baseRate === null) { prob = 0.5 }
      else { const effectiveDow = dowRate !== null ? dowRate : baseRate; prob = effectiveDow * 0.50 + baseRate * 0.28 + (streakSum/3) * 0.15 + todayBonus * 0.07 }
    ...
    stats.sort((a,b) => b.prob - a.prob); latestPredictions = stats
  ...
  function renderPredictionUI(top3) {
    ...
    document.getElementById('prediction-grid').innerHTML = top3.map(item => {
  ...
  function getCycleScores(year, month) {
    ...
    for (let dd = 1; dd <= daysInMonth; dd++) {
      ...
      const score = getCalculatedScoreForDate(ds), seg = dd <= 10 ? 0 : dd <= 20 ? 1 : 2
      parts[seg] += score; total += score
    }
  ...
  function updateEfficiencyWidget() {
    ...
    const cycleYear = currentCycleStart.getFullYear(), cycleMonth = currentCycleStart.getMonth() + 1
    ...
    const segStarts = [1,11,21], segEnds = [10,20,daysInMonth]
    let html = ''
    for (let i = 0; i < 3; i++) {
      const start = segStarts[i], end = segEnds[i], segDays = end - start + 1, maxPts = MISSIONS.length * segDays
      const pts = curScores.parts[i].toFixed(1), pct = ((curScores.parts[i] / maxPts) * 100).toFixed(1)
      ...
      const t2 = SEG_THEMES[i], isCur = i === curSeg
      const badge = isCur ? `<span style="font-size:10px;background:${t2.color};color:white;padding:2px 6px;border-radius:4px;margin-left:8px;">NOW</span>` : ''
      const cardStyle = isCur ? `--m-color:${t2.color};--m-rgb:${t2.rgb};border-color:var(--border-2);box-shadow:var(--shadow-sm);` : `--m-color:${t2.color};--m-rgb:${t2.rgb};`
    ...
    const totalMaxPts = MISSIONS.length * daysInMonth, curTotalPct = ((curScores.total / totalMaxPts) * 100).toFixed(1)
    const prevTotalMaxPts = MISSIONS.length * prevScores.daysInMonth, prevTotalPct = ((prevScores.total / prevTotalMaxPts) * 100).toFixed(1)
  ...
  document.getElementById('efficiency-grid').addEventListener('click', e => {
    ...
    if (!card) return
    const type = card.dataset.openModal, idx = card.dataset.modalIdx !== '' ? parseInt(card.dataset.modalIdx) : null
  ...
  function updateCalendarWidget(dateStr) {
    ...
    document.getElementById('cal-subtitle').innerText = `${daysInMonth}-day month · 10 · 10 · ${daysInMonth - 20}`
    const today = getPSTDate(), todayDay = (y === today.getFullYear() && m === today.getMonth()+1) ? today.getDate() : -1
    function secScore(start10, end10) { let s = 0; for (let dd = start10; dd <= Math.min(end10, daysInMonth); dd++) s += getCalculatedScoreForDate(`${y}-${String(m).padStart(2,'0')}-${String(dd).padStart(2,'0')}`); return s.toFixed(1) }
    const todaySec = todayDay > 0 ? (todayDay <= 10 ? 0 : todayDay <= 20 ? 1 : 2) : -1
    let html = ''
    for (let sec = 0; sec < 3; sec++) {
      const start = sec * 10 + 1, end = Math.min(start + 9, daysInMonth)
      const isActiveSec = sec === todaySec, score = secScore(start, end)
      const maxSec = MISSIONS.length * (end - start + 1), pct = maxSec > 0 ? ((parseFloat(score) / maxSec) * 100).toFixed(0) : 0
      ...
      for (let dd = start; dd <= end; dd++) {
        const dateObj = new Date(y, m-1, dd), isSun = dateObj.getDay() === 0, isTod = dd === todayDay, isPast = dd < todayDay
        let cls = 'cal-brick'
        if (isTod) cls += ' today'; else if (isPast) cls += ' past'; else cls += ' future'
        if (isSun) cls += ' sunday'
        html += `<div class="${cls}" title="${shortMonths[m-1]} ${dd}">${dd}</div>`
      }
      for (let dd = end + 1; dd < start + 10 && dd > daysInMonth; dd++) html += `<div class="cal-brick" style="opacity:0;pointer-events:none;"></div>`
  ...
  function updateWeekWidget(dateStr) {
    ...
    const today = new Date(y, m-1, d), dow = today.getDay()
    ...
    const eow = new Date(weekStart); eow.setDate(weekStart.getDate() + 6)
    document.getElementById('week-widget-label').innerText = `${monthShort[weekStart.getMonth()]} ${weekStart.getDate()} – ${monthShort[eow.getMonth()]} ${eow.getDate()}`
    document.getElementById('week-cells').innerHTML = cells
  }

  // ── Momentum / perfect day ────────────────────────────────────────────────────
  function getDailyMomentumValue(ds) { const s = getCalculatedScoreForDate(ds); return s <= 0 ? 0 : s >= MISSIONS.length ? 1 : 0.5 }
  function getMomentumCoefficient() {
    const today = getPSTDate(); let ws = 0, wt = 0
    for (let i = 0; i < 7; i++) { const d = new Date(today); d.setDate(d.getDate() - i); const w = 7 - i; ws += getDailyMomentumValue(formatPSTDate(d)) * w; wt += w }
    return wt > 0 ? ws / wt : 0
  }
  function isPerfectDay(ds) {
    const score = getCalculatedScoreForDate(ds); if (score > 5.0) return true
    const ml = logs[ds]; if (!ml?.missions) return false
    return Object.values(ml.missions).filter(s => s === 'completed' || s === 'partial').length === MISSIONS.length
  }
  function getPerfectDayStreak() {
    const today = getPSTDate(); let streak = 0, checkDate = new Date(today)
  ...
  function getRecoveryRate() {
    ...
    let times = [], lastBreak = null
    for (const ds of loggedDates) {
      if (!isPerfectDay(ds)) { lastBreak = new Date(ds) }
      else if (lastBreak) { times.push(Math.ceil((new Date(ds) - lastBreak) / 86400000)); lastBreak = null }
    }
    return times.length === 0 ? 0 : Math.round(times.reduce((a,b) => a+b, 0) / times.length * 10) / 10
  }
  ...
  function getPeriodDetailStats(startD, daysCount) {
    ...
    for (let i = 0; i < daysCount; i++) {
      ...
      const ds = formatPSTDate(d), log = logs[ds] || {}, ml = log.missions || {}
      stats.score += getCalculatedScoreForDate(ds); stats.focus += parseFloat(log.deepFocus || 0)
  ...
  function openDetailModal(type, index) {
    ...
    if (type === 'seg') { startD = new Date(cycleYear, cycleMonth-1, segStarts[index]); days = segEnds[index] - segStarts[index] + 1; title = `${SEG_THEMES[index].name} Insights`; accent = SEG_THEMES[index].color }
    ...
    MISSIONS.forEach(m => {
      const val = stats.missions[m.id], pct = (val / days) * 100
  ...
  function closeDetailModal() {
    ...
    modal.classList.remove('active'); modal._releaseTrap?.(); currentModalExportData = null
  }
  function exportModalCSV() {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `vanguard_${title.replace(/\s+/g,'_').toLowerCase()}_stats.csv`
    document.body.appendChild(a); a.click(); document.body.removeChild(a)
  }

  document.getElementById('stats-modal').addEventListener('click', e => { if (e.target === e.currentTarget) closeDetailModal() })
  ...
  function updateWeekNumberBar() {
    const bar = document.getElementById('week-number-bar'), today = getPSTDate()
    ...
    const base = new Date(today); base.setDate(today.getDate() + daysUntilNextSunday + weekBarOffset * 7); base.setHours(0,0,0,0)
    let html = ''
    for (let i = 0; i < 3; i++) {
      const weekStart = new Date(base); weekStart.setDate(base.getDate() + i * 7)
  ...
  document.getElementById('week-bar-prev').addEventListener('click', () => { if (weekBarOffset > 0) { weekBarOffset -= 3; if (weekBarOffset < 0) weekBarOffset = 0; updateWeekNumberBar() } })
  document.getElementById('week-bar-next').addEventListener('click', () => { weekBarOffset += 3; updateWeekNumberBar() })
  ...
  function updateCycleGoalInput() {
    const csKey = formatPSTDate(currentCycleStart), csEnd = getCycleEnd(currentCycleStart)
    const mo = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
    document.getElementById('goal-cycle-label').textContent = `${mo[currentCycleStart.getMonth()]} ${currentCycleStart.getDate()} – ${mo[csEnd.getMonth()]} ${csEnd.getDate()}`
    const val = cycleGoals[csKey] || ''
    document.getElementById('inp-cycle-goal').value = val
    const cc = document.getElementById('goal-char-count'); cc.textContent = `${val.length} / 150`; cc.classList.toggle('warn', val.length > 120)
  }
  ...
  function checkCycleSummary() {
    const card = document.getElementById('cycle-summary-card'), body = document.getElementById('csc-body')
    if (!card || !body) return
    const today = getPSTDate(), csEnd = getCycleEnd(currentCycleStart)
    ...
    const nDays = getCycleDays(currentCycleStart), dates = []
    for (let i = 0; i < nDays; i++) { const d = new Date(currentCycleStart.getFullYear(),currentCycleStart.getMonth(),currentCycleStart.getDate()+i); dates.push(formatPSTDate(d)) }
    let cycleScore=0, bestScore=-1, bestDate=null, focusTotal=0
    const mCounts = {}; MISSIONS.forEach(m => mCounts[m.id] = 0)
    dates.forEach(ds => {
      const s = getCalculatedScoreForDate(ds); cycleScore += s
      if (s > bestScore) { bestScore = s; bestDate = ds }
      focusTotal += parseFloat(logs[ds]?.deepFocus || 0)
      Object.entries(logs[ds]?.missions || {}).forEach(([mid, st]) => { mCounts[mid] = (mCounts[mid]||0) + (st==='completed'?1:st==='partial'?0.5:0) })
    })
    const prevCs = getPrevCycleStart(currentCycleStart), prevDays = getCycleDays(prevCs)
    let prevScore = 0; const prevCounts = {}; MISSIONS.forEach(m => prevCounts[m.id] = 0)
    for (let i = 0; i < prevDays; i++) {
      const d = new Date(prevCs.getFullYear(),prevCs.getMonth(),prevCs.getDate()+i)
      const ds = formatPSTDate(d); prevScore += getCalculatedScoreForDate(ds)
      Object.entries(logs[ds]?.missions || {}).forEach(([mid, st]) => { prevCounts[mid] = (prevCounts[mid]||0) + (st==='completed'?1:st==='partial'?0.5:0) })
    }
    let bestM=null, bestMDelta=-Infinity, worstM=null, worstMDelta=Infinity
    MISSIONS.forEach(m => { const delta = (mCounts[m.id]||0)-(prevCounts[m.id]||0); if (delta>bestMDelta){bestMDelta=delta;bestM=m} if (delta<worstMDelta){worstMDelta=delta;worstM=m} })
    const diff = cycleScore - prevScore, diffStr = diff===0?'=':(diff>0?`+${diff.toFixed(1)}`:diff.toFixed(1))
    const diffCls = diff>0?'csc-up':diff<0?'csc-down':'csc-same'
    const bestDateLabel = bestDate ? new Date(bestDate+'T12:00:00').toLocaleDateString('en-US',{month:'short',day:'numeric'}) : '—'
    ...
    `
    card.style.display = ''
    document.getElementById('csc-dismiss').onclick = () => { localStorage.setItem(dismissKey,'1'); card.style.display='none' }
  }

  // ── Strategic insights ────────────────────────────────────────────────────────
  function updateStrategicInsights() {
    const loggedDates = Object.keys(logs).sort(), container = document.getElementById('strategic-insights-grid')
    if (!container) return
    if (!loggedDates.length) { container.innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-4);font-weight:600;">Track a few days to unlock insights.</div>'; return }
    const n = loggedDates.length, allScores = loggedDates.map(d => getCalculatedScoreForDate(d))
    const avgScore = (allScores.reduce((a,b) => a+b, 0)/n).toFixed(2), maxScore = Math.max(...allScores)
    const totalPts = allScores.reduce((a,b) => a+b, 0).toFixed(1)
    const perfectStreak = getPerfectDayStreak(), perfectDays = loggedDates.filter(d => isPerfectDay(d)).length, recoveryRate = getRecoveryRate()
    let earlyScores=[], noEarlyScores=[], planScores=[], noPlanScores=[], onTrackScores=[], offTrackScores=[]
    loggedDates.forEach(d => {
      const s = getCalculatedScoreForDate(d)
      if (logs[d]?.earlyWake) earlyScores.push(s); else noEarlyScores.push(s)
      if (logs[d]?.planning)  planScores.push(s); else noPlanScores.push(s)
      const ot = logs[d]?.onTrack; if (ot==='full'||ot==='partial') onTrackScores.push(s); else offTrackScores.push(s)
    })
    const avgE = earlyScores.length>0 ? earlyScores.reduce((a,b)=>a+b,0)/earlyScores.length : 0
    const avgNE = noEarlyScores.length>0 ? noEarlyScores.reduce((a,b)=>a+b,0)/noEarlyScores.length : 0
    const avgP = planScores.length>0 ? planScores.reduce((a,b)=>a+b,0)/planScores.length : 0
    const avgNP = noPlanScores.length>0 ? noPlanScores.reduce((a,b)=>a+b,0)/noPlanScores.length : 0
    const avgOT = onTrackScores.length>0 ? onTrackScores.reduce((a,b)=>a+b,0)/onTrackScores.length : 0
    const avgOff = offTrackScores.length>0 ? offTrackScores.reduce((a,b)=>a+b,0)/offTrackScores.length : 0
    const totalFocus = loggedDates.reduce((s,d) => s+parseFloat(logs[d]?.deepFocus||0), 0), avgFocus = (n>0?totalFocus/n:0).toFixed(1)
    let correctPred=0, totalPred=0
    loggedDates.forEach(d => { const preds = logs[d]?.predictions; if (!preds) return; preds.forEach(p => { totalPred++; if ((logs[d]?.missions?.[p.id]==='completed') === p.prob>=50) correctPred++ }) })
    const accuracyPct = totalPred>0 ? Math.round((correctPred/totalPred)*100) : 0
    const missionStats = MISSIONS.map(m => {
      let comp=0, part=0, streak=0, curS=0
      ...
      for (let i=0;i<sorted.length;i++) {
        if (i===0){curS=1;streak=1;continue}
        if (Math.round((new Date(sorted[i])-new Date(sorted[i-1]))/86400000)===1){curS++;streak=Math.max(streak,curS)} else curS=1
      }
    ...
    loggedDates.forEach(d => { const [y,mo,dy]=d.split('-').map(Number), dow=new Date(y,mo-1,dy).getDay(); dowSums[dow]+=getCalculatedScoreForDate(d); dowCounts[dow]++ })
    ...
    const dowNames=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'], dowColors=['var(--rose)','var(--cyan)','var(--green)','var(--accent)','var(--gold)','var(--violet)','var(--teal)']
    const momentum=(getMomentumCoefficient()*100).toFixed(0), activeDays=allScores.filter(s=>s>0).length, consistencyPct=n>0?Math.round((activeDays/n)*100):0
    ...
    for (let i=0;i<7;i++) {
      const val = dowCounts[i]>0?dowSums[i]/dowCounts[i]:0, hp = dowMax>0?Math.round((val/dowMax)*100):0
  ...
  function updateDailyWidgets(dateStr) {
    ...
    displayPts.innerText = score.toFixed(1); displayPts.style.color = score===0?'var(--text-3)':getIntensityColor(score)
    if (inpDate.value === dateStr) {
      const noteVal = logs[dateStr]?.note || ''; inpNote.value = noteVal
      noteCharCount.textContent = `${noteVal.length} / 200`; noteCharCount.classList.toggle('warn', noteVal.length>170)
      const energy = logs[dateStr]?.energy || null
      ;['low','medium','high'].forEach(lvl => {
        const btn = document.getElementById(`btn-energy-${lvl}`)
        btn.className = `energy-btn${energy===lvl?` active-${lvl}`:''}`;  btn.setAttribute('aria-pressed', energy===lvl?'true':'false')
      })
      inpDeepFocus.value = logs[dateStr]?.deepFocus || ''
      const isEarly = !!logs[dateStr]?.earlyWake
      btnEarlyWake.classList.toggle('active', isEarly); btnEarlyWake.innerHTML = isEarly?'<i class="fa-solid fa-check"></i> Early Wake Up':'<i class="fa-regular fa-clock"></i> Early Wake Up'
      const isPlan = !!logs[dateStr]?.planning
      btnPlanning.classList.toggle('active-planning', isPlan); btnPlanning.innerHTML = isPlan?'<i class="fa-solid fa-check"></i> Planned':'<i class="fa-solid fa-calendar-plus"></i> Planning'
    ...
    updateMonthProgress(dateStr); updateWeekWidget(dateStr); updateCalendarWidget(dateStr); updateMissionRing(dateStr)
  }

  // ── Mission ring ──────────────────────────────────────────────────────────────
  function updateMissionRing(dateStr) {
    const svg = document.getElementById('ring-svg'), barsEl = document.getElementById('ring-bars')
    ...
    const CX=60, CY=60, SW=4, RADII=[55,48,41,34,27,20,13]
    MISSIONS.forEach((m, i) => {
      const r=RADII[i], C=2*Math.PI*r, status=missions[m.id]||null
      const isComp=status==='completed', isPart=status==='partial'
      const track = document.createElementNS('http://www.w3.org/2000/svg','circle')
      track.setAttribute('class','orbital-track'); track.setAttribute('cx',CX); track.setAttribute('cy',CY); track.setAttribute('r',r)
      track.setAttribute('stroke',m.color); track.setAttribute('stroke-width',SW); track.setAttribute('fill','none'); track.setAttribute('opacity','0.1')
      svg.appendChild(track)
      if (!isComp && !isPart) return
      const arc = document.createElementNS('http://www.w3.org/2000/svg','circle')
      arc.setAttribute('class',`orbital-arc ${isComp?'completed':'partial'}`); arc.setAttribute('cx',CX); arc.setAttribute('cy',CY); arc.setAttribute('r',r)
      arc.setAttribute('stroke',m.color); arc.setAttribute('stroke-width',SW); arc.setAttribute('fill','none')
      if (isComp) { arc.setAttribute('stroke-dasharray',`${C} 0`); arc.setAttribute('stroke-dashoffset','0'); arc.style.filter=`drop-shadow(0 0 5px ${m.color})` }
      else { const half=C/2; arc.setAttribute('stroke-dasharray',`${half} ${half}`); arc.setAttribute('stroke-dashoffset',`${-half/2}`) }
      svg.appendChild(arc)
    })
    barsEl.innerHTML = ''
    MISSIONS.forEach(m => {
      const status=missions[m.id]||null, fillPct=status==='completed'?100:status==='partial'?50:0
      const glow=fillPct===100?`box-shadow:0 0 5px ${m.color}55`:'', sym=fillPct===100?'✓':fillPct===50?'½':'–', symColor=fillPct>0?m.color:'var(--text-4)'
      const row=document.createElement('div'); row.className='ring-bar-row'; row.title=`${m.name}: ${status||'—'}`
  ...
  function updateInsights() {
    ...
    for (const dateStr in logs) { const score=getCalculatedScoreForDate(dateStr); if (score>0){const [y,m,d]=dateStr.split('-').map(Number),day=new Date(y,m-1,d).getDay();ds[day]+=score;dc[day]++} }
    let bestDay=-1, maxAvg=-1; for (let i=0;i<7;i++) if (dc[i]>0&&ds[i]/dc[i]>maxAvg){maxAvg=ds[i]/dc[i];bestDay=i}
    ...
    for (const m of MISSIONS) {
      const dates=Object.keys(logs).filter(d=>logs[d]?.missions?.[m.id]==='completed').sort(); tc[m.id]=dates.length; let maxS=0,curS=0
      for (let i=0;i<dates.length;i++){if(i===0){curS=1;maxS=1;continue} if(Math.round((new Date(dates[i])-new Date(dates[i-1]))/86400000)===1){curS++;maxS=Math.max(maxS,curS)}else curS=1}
      ls[m.id]=maxS
    }
    const ss=Object.entries(ls).sort((a,b)=>b[1]-a[1]); document.getElementById('lbl-best').innerText=ss[0][1]>0?MISSIONS.find(m=>m.id===ss[0][0]).name:'—'
    const sc2=Object.entries(tc).sort((a,b)=>a[1]-b[1]); document.getElementById('lbl-worst').innerText=sc2.length>0?MISSIONS.find(m=>m.id===sc2[0][0]).name:'—'
    updateEnergyCorrelation()
  }

  function updateEnergyCorrelation() {
    const groups={low:[],medium:[],high:[]}
    Object.entries(logs).forEach(([date,log])=>{if(!log?.energy||!groups[log.energy])return;groups[log.energy].push(getCalculatedScoreForDate(date))})
    const fmt=arr=>arr.length?`${(arr.reduce((a,b)=>a+b,0)/arr.length).toFixed(1)} pts (${arr.length}d)`:'—'
  ...
  function calcYearProgress(nowDate) {
    const start = new Date(nowDate.getFullYear(),0,0), doy = Math.floor((nowDate-start)/86400000)
    const yr = nowDate.getFullYear(), total = (yr%4===0&&yr%100!==0)||yr%400===0?366:365
    dispDay.innerText = doy; dispPct.innerText = ((doy/total)*100).toFixed(1)
  }

  // ── Shortcuts modal ───────────────────────────────────────────────────────────
  let _plusCount=0, _plusTimer=null
  const shortcutsModal = document.getElementById('shortcuts-modal')
  function openShortcuts() { shortcutsModal.classList.add('active'); shortcutsModal._releaseTrap=window.trapFocus(shortcutsModal) }
  function closeShortcuts() { shortcutsModal.classList.remove('active'); shortcutsModal._releaseTrap?.() }
  function toggleShortcuts() { shortcutsModal.classList.contains('active') ? closeShortcuts() : openShortcuts() }
  ...
  ;['low','medium','high'].forEach(lvl => {
    document.getElementById(`btn-energy-${lvl}`).addEventListener('click', () => {
      ...
      saveToLocal(); updateDailyWidgets(date); updateEnergyCorrelation()
    })
  })
  inpNote.addEventListener('input', e => {
    const val=e.target.value; noteCharCount.textContent=`${val.length} / 200`; noteCharCount.classList.toggle('warn',val.length>170)
    if (!logs[inpDate.value]) logs[inpDate.value]={missions:{}}
    if (val.trim()) logs[inpDate.value].note=val; else delete logs[inpDate.value].note
    saveToLocal()
  })
  ...
  btnEarlyWake.addEventListener('click', () => {
    if (!logs[inpDate.value]) logs[inpDate.value]={missions:{}}
    logs[inpDate.value].earlyWake=!logs[inpDate.value].earlyWake; saveToLocal()
    try {
      const _dk=inpDate.value, _mData=JSON.parse(localStorage.getItem('mastery_data')||'{}')
      ...
      localStorage.setItem('mastery_data',JSON.stringify(_mData)); saveCloudKey('mastery_data',_mData); _nexusSync.broadcast('VANGUARD')
    } catch(_){}
  ...
  document.getElementById('btn-export-predictions').addEventListener('click', () => {
    ...
    const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'}),url=URL.createObjectURL(blob)
    const a=document.createElement('a');a.href=url;a.download='vanguard_predictions.csv';document.body.appendChild(a);a.click();document.body.removeChild(a)
  })
  document.getElementById('inp-cycle-goal').addEventListener('input', e => {
    const val=e.target.value, csKey=formatPSTDate(currentCycleStart)
    const cc=document.getElementById('goal-char-count'); cc.textContent=`${val.length} / 150`; cc.classList.toggle('warn',val.length>120)
    if (val.trim()) cycleGoals[csKey]=val; else delete cycleGoals[csKey]; saveCycleGoals()
  })
  ...
  const _keyHandler = e => {
    ...
    if (e.key==='Escape'){closeShortcuts();closeDetailModal()}
    ...
    if (e.key==='+'){
      _plusCount=(_plusCount||0)+1; clearTimeout(_plusTimer)
      _plusTimer=setTimeout(()=>{
        const date=inpDate.value
        if (!logs[date]) logs[date]={missions:{}}
        if (_plusCount===1) logs[date].onTrack='partial'; else if (_plusCount===2) logs[date].onTrack='full'; else delete logs[date].onTrack
        _plusCount=0; saveToLocal(); updateDailyWidgets(date)
        const btn=document.getElementById('btn-on-track'); if (btn){btn.style.transition='box-shadow 0.2s';btn.style.boxShadow='0 0 0 3px var(--accent)';setTimeout(()=>{btn.style.boxShadow=''},900)}
      },400)
    }
    if (e.key==='p'||e.key==='P'){btnPlanning.click();btnPlanning.scrollIntoView({behavior:'smooth',block:'center'})}
    if (e.key==='e'||e.key==='E'){btnEarlyWake.click();btnEarlyWake.scrollIntoView({behavior:'smooth',block:'center'})}
    if (e.key==='o'||e.key==='O'){const btn=document.getElementById('btn-on-track');if(btn){btn.scrollIntoView({behavior:'smooth',block:'center'});btn.style.transition='box-shadow 0.2s';btn.style.boxShadow='0 0 0 3px var(--accent)';setTimeout(()=>{btn.style.boxShadow=''},1200)}}
    if (e.key==='a'||e.key==='A'){const note=document.getElementById('inp-note');if(note){note.focus();note.select();e.preventDefault()}}
    if (e.key==='i'||e.key==='I'){const siw=document.getElementById('strategic-insights-widget');if(siw){siw.scrollIntoView({behavior:'smooth',block:'start'});siw.style.transition='box-shadow 0.2s';siw.style.boxShadow='0 0 0 3px var(--accent)';setTimeout(()=>{siw.style.boxShadow=''},1200)}}
    const num=parseInt(e.key); if (num>=1&&num<=MISSIONS.length){const mid=MISSIONS[num-1].id,date=inpDate.value,cell=table.querySelector(`td[data-date="${date}"][data-mid="${mid}"]`);if(cell)toggleMission(cell)}
  }
  ...
  try {
    ...
    if ((_mData[_todayStr]||[]).includes('wakeup')&&!logs[_todayStr]?.earlyWake){
      ...
      logs[_todayStr].earlyWake=true; saveToLocal()
    }
  } catch(_){}
  ...
  renderGrid(); updateInsights(); calculateAI_Predictions()
