...
export async function init(container, _user) {
  ...
  function updateTemporal() {
    ...
    document.getElementById('tw-date').textContent =
    ...
    document.getElementById('tw-weekrange').textContent =
      `${months[sow.getMonth()]} ${sow.getDate()} – ${months[eow.getMonth()]} ${eow.getDate()}`
    ...
    document.getElementById('tw-session').textContent =
      hh < 12 ? 'MORNING' : hh < 17 ? 'AFTERNOON' : hh < 21 ? 'EVENING' : 'NIGHT'
  ...
  function updateMetrics(data) {
    ...
    document.getElementById('m-weakday-avg').textContent =
      weakIdx >= 0 ? `avg ${weakVal.toFixed(1)} rituals` : 'not enough data'
    ...
    document.getElementById('m-topritual').textContent =
      top && top[1] > 0 ? ritualLabels[top[0]] || top[0] : '—'
    document.getElementById('m-topritual-count').textContent =
      top && top[1] > 0 ? `${top[1]} completions` : 'all-time completions'
    ...
    if (perfKeys.length) {
      ...
      document.getElementById('m-sinceperfect-sub').textContent =
        diff === 0 ? 'perfect day!' : 'days ago'
    } else {
    ...
    document.getElementById('m-monthvs-sub').textContent = lastMS
      ? `${diffM >= 0 ? '+' : ''}${diffM} vs last month`
      : 'no last-month data'
  ...
  function launchConfetti() {
    ...
    for (let i = 0; i < 180; i++) {
      ...
      parts.push({
        ...
        opacity: 1,
        fadeStart: canvas.height * (0.55 + Math.random() * 0.25),
      })
    }
    function tick() {
  ...
  function buildBrickWall() {
    ...
    const td = _calToday.getDate(),
      tm = _calToday.getMonth(),
      ty = _calToday.getFullYear()
  ...
  function openDayModal(year, month, day) {
    ...
    const isToday =
      year === _calToday.getFullYear() &&
      month === _calToday.getMonth() &&
      day === _calToday.getDate()
    const isPast =
      dateObj < new Date(_calToday.getFullYear(), _calToday.getMonth(), _calToday.getDate())
    const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`

    document.getElementById('dModalTitle').textContent =
      `${dayOfWeek}, ${CAL_MONTH_NAMES[month]} ${day}`
  ...
  function _keyHandler(e) {
    ...
    if (navMap[key] !== undefined) {
      location.hash = navMap[key]
      return
    }
    const energyMap = { l: 'low', m: 'medium', h: 'high' }
    if (energyMap[key]) {
      const cur = saveEnergy(energyMap[key])
      renderEnergyButtons(cur)
      return
    }
    const ritualMap = {
      1: 'english',
    ...
    if (key === 'd') {
      document.getElementById('ritualGrid')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
      return
    }
    if (key === 'p') {
    ...
    if (key === 'c') {
      const _t = new Date()
      calViewYear = _t.getFullYear()
      calViewMonth = _t.getMonth()
      buildBrickWall()
      document
        .querySelector('.brick.today')
