...
export async function init(container, user) {
  ...
  document.getElementById('mwMasteryWidget').addEventListener('click', () => {
    if (isMobile()) location.hash = 'mastery'
  })
  document.getElementById('mwVanguardWidget').addEventListener('click', () => {
    if (isMobile()) location.hash = 'vanguard'
  })
  ...
  function updateTimeMatrix() {
    ...
    document.getElementById('monthProgressPct').textContent =
      `${Math.round((completedDays / daysInMonth) * 100)}% COMPLETE`
  }
  ...
  function buildMasteryCard(m) {
    ...
    if (!m.hasData)
      return cardShell(
        opts,
        `<div class="no-data">Data Not Found — Launch hub to initialize.</div>`
      )
    const chips = RITUAL_KEYS.map(
      id =>
        `<span class="chip ${m.completedIds.includes(id) ? 'done' : 'miss'}">${RITUAL_NAMES[id]}</span>`
    ).join('')
    return cardShell(
      opts,
      `
  ...
  function renderApp() {
    ...
    if (fill) {
      ...
      fill.style.boxShadow = `0 0 8px ${color}80`
    }
    ...
    document.getElementById('miniBars').innerHTML = [
      ...
      .map(
        b =>
          `<div class="mini-bar-item"><span class="mini-bar-label">${b.label}</span><div class="mini-bar-track"><div class="mini-bar-fill" style="width:${b.pct}%;background:${b.color};box-shadow:0 0 8px ${b.color}60;"></div></div><span class="mini-bar-pct" style="color:${b.color}">${b.pct}%</span></div>`
      )
      .join('')
  ...
  function getYesterdayVanguardPct() {
    ...
    const db = safeJSON('vanguard-logs', {}),
      dayLog = db[yesterday] || {},
      missions = dayLog.missions || {}
    const completedCount = VANGUARD_MISSIONS.filter(id => missions[id] === 'completed').length
    const partialCount = VANGUARD_MISSIONS.filter(id => missions[id] === 'partial').length
    return {
      pct: Math.round(((completedCount + partialCount * 0.5) / VANGUARD_MISSIONS.length) * 100),
      hasData: Object.keys(dayLog).length > 0,
    }
  }

  function renderDayDelta(m, v, globalPct) {
    const yM = getYesterdayMasteryPct(),
      yV = getYesterdayVanguardPct()
    const yRelevant = [yM, yV].filter(a => a.hasData)
    const yGlobal =
      yRelevant.length > 0
        ? Math.round(yRelevant.reduce((acc, a) => acc + a.pct, 0) / yRelevant.length)
        : null
    const arrowEl = document.getElementById('deltaArrow'),
      pctEl = document.getElementById('deltaPct')
    const subEl = document.getElementById('deltaSub'),
      barsEl = document.getElementById('deltaBars')
    ...
    pctEl.textContent = isFlat ? 'FLAT' : isUp ? `+${Math.abs(delta)}%` : `-${Math.abs(delta)}%`
    pctEl.style.color = color
    ...
    const appColors = ['#fbbf24', '#c084fc'],
      appLabels = ['M', 'V']
    const todayPcts = [m.pct, v.pct],
      yestPcts = [yM.pct, yV.pct]
    barsEl.innerHTML = [0, 1]
      .map(i => {
        const maxH = 32,
          tH = Math.round((todayPcts[i] / 100) * maxH),
          yH = Math.round((yestPcts[i] / 100) * maxH),
          c = appColors[i]
  ...
  function calcStreak() {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      const vDone =
        vEntry &&
        Object.values(vEntry.missions || {}).filter(s => s === 'completed' || s === 'partial')
          .length >= 3
      if (mDone && vDone) {
        streak++
        d.setDate(d.getDate() - 1)
  ...
  function renderMiniWidgets(m, v) {
    ...
    document.getElementById('mwVanguardStat').textContent =
      v.partial > 0
    ...
    document.getElementById('mwStreakBar').style.width =
      Math.min(Math.round((streak / STREAK_GOAL) * 100), 100) + '%'
  }
  ...
  function isDoneOnTrack(dateKey) {
    ...
    return (
      Array.isArray(raw) &&
      raw.some(item => (typeof item === 'string' ? item : item?.date) === dateKey)
    )
  }
  ...
  function getWeekData(tab, weekOffset = 0) {
    const dates = getWeekDates(weekOffset),
      today = todayKey()
    const checkFn =
      tab === 'planning' ? isDonePlanning : tab === 'tracking' ? isDoneTracking : isDoneOnTrack
  ...
  function renderWeekPanel() {
    ...
    const curr = getWeekData(activeWeekTab, 0),
      prev = getWeekData(activeWeekTab, -1)
  ...
  const _visibilityHandler = () => {
    if (document.hidden) {
      ...
      _paused = false /* frame() re-starts on next tick via rAF */
    }
  }
